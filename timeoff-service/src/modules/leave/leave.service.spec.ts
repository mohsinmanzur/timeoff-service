...
import { HcmInsufficientBalanceException } from '../hcm-client/exceptions/hcm.exceptions';
...
describe('LeaveService', () => {
  ...
  describe('requestTimeOff', () => {
    it('happy path: creates request, increments pendingDays, calls HCM deduct', async () => {
      ...
      const hcm = buildHcm();
      hcm.deductBalance.mockResolvedValue({
        success: true,
        remainingBalance: 10,
      ...
      expect(hcm.deductBalance).toHaveBeenCalledWith(
        'EMP-001',
        'LOC-NYC',
        5,
        savedRequest.id,
    ...
    it('insufficient local balance: throws InsufficientBalanceException, no HCM call', async () => {
      ...
      await expect(
        service.requestTimeOff({ ...dto, daysRequested: 5 }),
    ...
    it('HCM returns INSUFFICIENT_BALANCE: rolls back, throws InsufficientBalanceException', async () => {
      ...
      const qr = buildQueryRunner([balance]);
      const hcm = buildHcm();
      hcm.deductBalance.mockRejectedValue(
        new HcmInsufficientBalanceException('HCM insufficient'),
      );
      ...
      await expect(service.requestTimeOff(dto)).rejects.toThrow(
        InsufficientBalanceException,
      );
      expect(qr.rollbackTransaction).toHaveBeenCalled();
  ...
  describe('approveRequest', () => {
    ...
    it('request not found: throws RequestNotFoundException', async () => {
      ...
      await expect(
        service.approveRequest('missing-id', 'MGR-007'),
      ).rejects.toThrow(RequestNotFoundException);
      expect(qr.rollbackTransaction).toHaveBeenCalled();
    });
  ...
  describe('rejectRequest', () => {
    it('happy path: decrements pendingDays, calls restoreBalance, status = REJECTED', async () => {
      ...
      expect(result.reviewedBy).toBe('MGR-007');
      expect(hcm.restoreBalance).toHaveBeenCalledWith(
        'EMP-001',
        'LOC-NYC',
        5,
        'req-1',
  ...
  describe('cancelRequest', () => {
    it('non-owner tries to cancel: throws UnauthorizedCancellationException', async () => {
      ...
      await expect(
        service.cancelRequest('req-1', 'DIFFERENT-EMP'),
      ).rejects.toThrow(UnauthorizedCancellationException);
      expect(qr.rollbackTransaction).toHaveBeenCalled();
    });
  ...
  describe('getBalance', () => {
    ...
    it('balance is stale (> 5 min): returns local data and triggers background HCM sync', async () => {
      ...
      const service = new LeaveService(ds, hcm);
      const syncSpy = jest
        .spyOn(service, 'syncBalanceFromHcm')
    ...
    it('no local balance: calls syncBalanceFromHcm and returns the synced balance', async () => {
      const syncedBalance = makeBalance({
        totalDays: 25,
        usedDays: 0,
        pendingDays: 0,
      });
      ...
      jest
        .spyOn(service, 'syncBalanceFromHcm')
        .mockResolvedValue(syncedBalance);
  ...
  describe('syncBalanceFromHcm', () => {
    it('creates a new LeaveBalance when none exists', async () => {
      ...
      const txManager = {
        ...
        save: jest
          .fn()
          .mockResolvedValue({ ...createdBalance, lastSyncedAt: new Date() }),
      };
      ...
      ds.transaction = jest
        .fn()
        .mockImplementation((fn: (em: any) => Promise<any>) => fn(txManager));
      const service = new LeaveService(ds, hcm);
